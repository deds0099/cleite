                  ...
                  <TableBody>
                    {animais.map((animal) => (
                      ...
                      >
                        ...
                        <TableCell onClick={() => navigate(`/animal/${animal.id}`)}>
                          {format(fixDateDisplay(animal.data_nascimento), 'dd/MM/yyyy')}
                        </TableCell>
                        <TableCell onClick={() => navigate(`/animal/${animal.id}`)}>{animal.quantidade_partos}</TableCell>
                        <TableCell onClick={() => navigate(`/animal/${animal.id}`)}>
                          <span className={animal.status === 'Ativo' ? 'text-green-600' : 'text-red-600'}>
                            {animal.status}
                          </span>
                        </TableCell>
                        <TableCell>
