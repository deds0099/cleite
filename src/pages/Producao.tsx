...
export default function Producao() {
  ...
  const producaoHoje = registros
    .filter(r => {
      const dataRegistro = fixDateDisplay(r.data);
      return format(dataRegistro, 'yyyy-MM-dd') === format(hoje, 'yyyy-MM-dd') && r.periodo === 'Total';
    })
  ...
  const producaoPeriodo = registros
    .filter(registro => {
      const dataRegistro = fixDateDisplay(registro.data);
    ...
    <Layout>
      <div className="container mx-auto py-6 space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
                  ...
                  {registrosTotais
                    ...
                    .map((registro) => (
                      <TableRow key={registro.id}>
                        <TableCell>
                          {format(fixDateDisplay(registro.data), 'dd/MM/yyyy')}
                        </TableCell>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Histórico de Produção Individual</CardTitle>
              <CardDescription>
                Detalhe da produção individual por animal no período
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
