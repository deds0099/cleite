...
export default function Alertas() {
  ...
  const proximosPartos = animais?.filter(animal => 
    ...
    isBefore(new Date(), new Date(animal.data_proximo_parto)) &&
    differenceInDays(new Date(animal.data_proximo_parto), new Date()) <= 30
  ) || [];
  ...
  const proximasVacinas = animais?.flatMap(animal => 
    ...
    differenceInDays(new Date(vacina.data), new Date()) <= 30
                ...
                <Table>
                  ...
                  <TableBody>
                    {proximasVacinas.map((vacina, index) => (
                      <TableRow key={index}>
                        <TableCell>
                          {format(new Date(vacina.data), 'dd/MM/yyyy')}
                        </TableCell>
                        <TableCell>
                          <Button 
                            variant="link" 
