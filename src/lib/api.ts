...
export const addAnimal = async (animal: Omit<Animal, 'id' | 'created_at'>) => {
  ...
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      data_nascimento: new Date(animal.data_nascimento).toISOString(),
      data_proximo_parto: animal.data_proximo_parto ? new Date(animal.data_proximo_parto).toISOString() : null
    }])
    .select()
    .single();
...
export const addAlerta = async (alerta: Omit<Alerta, 'id' | 'created_at'>) => {
  ...
  const { data, error } = await supabase
    .from('alertas')
    .insert([{ ...alerta, user_id: session.user.id }])
    .select()
    .single();
  if (error) throw error;
  return data as Alerta;
